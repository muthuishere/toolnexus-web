...
<ctx-optimize>
  ...
  <store>Pre-built knowledge store at `~/ctxoptimize/toolnexus-web/` (config in `.ctxoptimize/` here).</store>
  ...
  `~/ctxoptimize/toolnexus-web/wiki/`. Output is parsed fact with exact file:line — cite it directly, do
  ...
  <no-local-store>Fresh clone with nothing at `~/ctxoptimize/toolnexus-web/`? Run `ctx-optimize up` —
